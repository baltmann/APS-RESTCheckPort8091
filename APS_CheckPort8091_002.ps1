...
$cmd.CommandText = "SELECT row_number() over (order by l.SiteId, IP) rn, l.SiteId, [Contact Code], IP FROM location l join sites s on s.SiteId=l.SiteId WHERE mac LIKE '000b%' AND enabled = 1 ORDER BY l.SiteId"
...
while ($reader.Read()) {
    $devices += [PSCustomObject]@{
        ...
        ContactCode = $reader["Contact Code"]
        IP          = $reader["IP"]
    }
}
$reader.Close()
